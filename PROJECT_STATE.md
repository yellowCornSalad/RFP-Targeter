...
## 🌿 브랜치 구조 (2026-06-02 정리)

| 브랜치 | 용도 | 위치 |
|---|---|---|
| **main** | 개발 | 활성 |
| **v1.1** ⭐ | **현재 안정 release** (IRIS 추가 전 마지막 정상 상태) | 라이브 복귀 지점 |
| v1.0 | 옛 안정 release | 참고용 |
| feat/iris-adapter | IRIS 통합 어댑터 작업장 | 격리 작업 |
| backup/before-iris-main | 보조 백업 (= v1.1 시작점) | 비상용 |
| backup/before-iris-v1.0 | 보조 백업 (= v1.0 시작점) | 비상용 |

**태그**: `stable/2026-06-02-main`, `stable/2026-06-02-v1.0`

**라이브 복귀 방법**: cron-job.org Body 의 `{"ref":"main"}` 을 `{"ref":"v1.1"}` 로 변경 → 즉시 옛 코드로 운영.
...
### 2026-06-01
- **크롤 상태 가시화 (하트비트 + 홈 배지 + 자동 빌드)** — [사용자 요청] 크롤완료 메시지를 끈 뒤 '살아있다' 신호 보완. (1) **일일 하트비트**: 매일 09시 KST `notify_daily_heartbeat` 1회 "✅ 지난 24h N회 정상 · 마지막 동기화 HH:MM" (`meta` 테이블 dedup, `daily_heartbeat_enabled`). (2) **홈 헤더 크롤 상태 배지**: `build_static.py`가 `fetch_log` 마지막 동기화 시각(UTC ISO)+24h 통계를 data.json 에 baking → `app.js`가 **조회 시점 기준 실시간** 색 전환(🟢<90분/🟡<180/🔴). 빌드 멈춰도 정직. (3) **`build_static.yml` workflow_run**: 크롤(Hourly Crawl) 완료 직후 자동 빌드 — GitHub schedule(:05) 지연·스킵 보정(cron-job.org 아님). + **init_db 트랜잭션 격리 픽스**: Supabase pgbouncer multi-statement abort 시 후속 DDL 이 swallow 되고 commit→rollback 되어 새 테이블(meta) 누락되던 버그 → DDL 마다 독립 트랜잭션.
- **소스 카드 NEW 배지 + '오늘 신규' KST 픽스** — [사용자 요청] NIPA 등에 오늘 신규가 있어도 소스 카드에 NEW 배지 미표시. 원인: `today_new_by_src` 를 서버(build_static)가 러너 **UTC** `datetime.now()` 로 계산 → KST 00~09시(=UTC 전날) 빌드에서 당일 신규를 0으로 셈. 해결: (a) build_static `today`→KST(Asia/Seoul), (b) UI 의 모든 '오늘' 판정(헤더·전체·소스·KPI·필터·카드 NEW)을 client KST 재계산(`todayKST`/`countNewToday`)으로 통일 → `toISOString`(UTC) 제거. 카드 NEW 와 소스 배지 항상 일치.

- **TRL 점수 인플레이션 차단** — [사용자 보고] 무관한 공고(K-수출전략품목, 중기부)가 TRL 95. 원인: 본문의 '인증' 단어가 `_TRL_KEYWORDS` 에서 TRL 9(상용화)로 오매핑 → 회사 TRL 8~9 와 gap 0 → 95. + 발주기관 default(MSS=8)도 회사 8 과 gap 0 → 95 인플레이션. 수정: (a) 노이즈 키워드(인증·보급·확산) TRL 키워드표에서 제거, (b) `from_default` 추정은 신뢰도 할인(최대 70). 실제 TRL 명시·강한 키워드(사업화·실증·상용화)만 고득점. `recompute_all_scores.py` 전체 재계산 → TRL 만점급 295건 중 0건, total 70~84 구간 18건 하향. 해당 공고 95→70 / total ~78→74.6. (※ 도메인 적합성은 TRL이 아닌 keyword 축 소관 — 별도 이슈)

- **LLM 본문 맥락 판단 — 도메인 적합성 + TRL (표시 1단계)** — [사용자 요청] 키워드 단순 매칭("사업화" 단어 존재만으로 TRL 95) 한계 보완. Claude Haiku 가 본문을 '읽고' (1) 회사 도메인 적합성(high/medium/low/none) (2) 실제 TRL 단계를 판단(hallucination 금지). `src/rfp_targeter/llm_assess.py` + `scripts/assess_contents.py`(llm_assess_json 캐시) + `build_summaries.yml`(audit 다음 단계, checkout v1.0→main, timeout 20→45). 대시보드: 카드 상단 적합성 배지 + 상세 "🤖 LLM 본문 판단" 블록(근거 포함). **1단계=표시만**(점수 미반영) — 검증 후 2단계 감점 예정. 결과: 활성 ~140건 평가 → 높음9·보통19·낮음55·무관57 = **85%가 무관(조사/교육/제조 용역)**. ※ 곁가지로 해결: ① **ANTHROPIC_API_KEY 미설정**(시크릿 추가 — ai_summary 도 여태 0/1431이던 게 부활), ② **크레딧 결제**(잔액 $0), ③ **6일째 누수 idle-in-transaction(pid 353091)** 종료 — announcement 스키마 변경(ADD COLUMN) 을 막던 락 점유 원인, ④ audit_contents `str(e)[:80]` 로그 잘림 → 전체 본문 로깅
