...
**점수 목적**: 공고가 컨소시엄·산학협력·다기관 연구를 **요구**하는지 판단 → 회사가 그걸 **만족**할 수 있는지 매칭. 만족 가능하면 만점 유지, 불가능하면 감점.

> 예시: 공고에 "대학 산학협력 필수" → 회사 KAIST 보유 시 **−5만 감점** (활용 가능), 대학 파트너 없으면 **−35** (큰 페널티).

**[2026-05-27 ecosystem 가점 추가]** 컨소시엄 신호 있는 공고에 회사 협력사 8곳 영역 매칭 시 가점.

```
baseline = 100  (단독 수행 가능 가정)

─── 1. 학계 참여 신호 ("대학·산학·교수·박사후·산학협력") ───
회사 대학 파트너 있음 (KAIST/가천대/세종대)    → −5
회사 대학 파트너 없음                          → −35

─── 2. 다기관 컨소시엄 신호 ("컨소시엄·공동연구·참여기관") ───
회사 max_partners ≥ 3 (현재 5)                  → −10
회사 max_partners < 3                            → −30

─── 3. 컨소시엄 신호 없음 ───
solo_capable=true (회사 단독 수행 능력 입증)   → 0  (만점 유지)
solo_capable 미설정                              → −10

─── 4. ecosystem 파트너 영역 매칭 (가점) ────
컨소시엄/학계 신호 있을 때만 발동.
회사 ecosystem_partners 의 domain 키워드가 본문에 등장:
  · SGA솔루션즈 (인증/서버보안)
  · 프라이빗테크놀로지 (NW보안/SDP)
  · NNSP (CDS)
  · 모니터랩 (RBI/WAF)
  ...
  · 이글루코퍼레이션 (관제)
  · 테이텀시큐리티 (추메/클라우드)
→ 매칭당 +3, cap +10 (협력 시너지 가능)
```

**현재 분포 (활성 보안 820건, v2)**:
| 점수 | 건수 | 의미 |
|---|---|---|
| 100점 | 353건 (43%) | 단독 수행 가능 + 신호 약함 |
| 95-99점 | 102건 (12%) | 대학 신호 + KAIST 활용 + ecosystem 가점 |
| 90-94점 | 253건 (31%) | 다기관 신호 + ecosystem 가점 |
| 85-88점 | 112건 (14%) | 대학 + 다기관 동시 |

→ unique 값 4개 → **11개**로 변별력 확장. 회사가 5개 파트너 + KAIST 공동특허 보유로 거의 모든 공고가 85+이지만 ecosystem 매칭으로 세분화.

##### 🏢 회사 협력 기반 (점수 가산 근거)

`profile.yaml.consortium`:
- **대학 파트너 3곳**: KAIST (공동특허 #28), 가천대, 세종대 (모의해킹 R&D)
- **기업 파트너 2곳**: 쏘마, 엔플러스랩 (모의해킹 컨소시엄)
- **max_partners**: 5 (3 → 5 확장, 2026-05-27)
- **solo_capable**: true (OFFen 4단계 platform 단독 수행 가능)

`profile.yaml.ecosystem_partners` (협력 시너지 후보 8곳): 위 매칭 표 참조
