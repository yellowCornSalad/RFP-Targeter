...
본문 신호                         조건                   감점
─────────────────────────────────────────────────────────────
"대학·산학·교수·박사후·산학협력"   대학 파트너 있음        −5
                                  대학 파트너 없음        −35

"컨소시엄·공동연구·참여기관·주관"  max_partners ≥ 3       −10
                                  max_partners < 3        −30

(컨소시엄 신호 없음)              solo_capable=true        0  (단독 수행 가정)
                                  solo_capable 미설정     −10
```
...
| 점수 | 건수 | 의미 |
|---|---|---|
| 100점 | 319건 (39%) | 컨소시엄 신호 약함 + 단독 수행 가능 |
| 95점 | 107건 (13%) | 대학 신호 있고 KAIST 활용 가능 |
| 90점 | 105건 (13%) | 다기관 신호 있고 5개 가능 범위 |
| 85점 | 289건 (35%) | 대학 + 다기관 신호 모두 |

> ⚠️ **변별력 한계**: 회사가 5개 파트너 + KAIST 공동특허 보유라 거의 모든 공고가 85+. 향후 신호 보강(주관/공동 역할 구분, N개 이상 기관 요구 인식)으로 분포 확장 검토.
